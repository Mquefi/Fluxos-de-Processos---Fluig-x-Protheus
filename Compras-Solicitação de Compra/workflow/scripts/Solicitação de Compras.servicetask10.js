function servicetask10(attempt, message) {
    try {
        ...
        log.warn('APROVADO: ' + hAPI.getCardValue('lb_aprovacao'));
        log.warn('COMPRADOR: ' + hAPI.getCardValue('fComprador'))

        var dados = {
            SOLICITANTE: hAPI.getCardValue('solicitante') + "",
            EMISSAO: hAPI.getCardValue('dataEmissao') + "",
            APROVADO: hAPI.getCardValue('lb_aprovacao') + "",
            COMPRADOR: hAPI.getCardValue('fComprador') + "",
            ITENS: vetor
        };
